%改进的梯度下降法，用于水文模型参数寻优
clear;clc;
%首先录入洪水数据，放入一个元胞数组data中，每个元胞代表一场洪水（由n列降水数据和1列流量数据组成）。以下录入了四场洪水为例，每场洪水前五列为五个雨量站数据，第六列为实测流量数据，时段长为1小时。
...
if state==0
    ...
    lvding=1:3;%率定选用洪水场次，可以选择单场，也可以选择多场，比如选择1~3场检验，第4场不参与率定因为它要留作检验
